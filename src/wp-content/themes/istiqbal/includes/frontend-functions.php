...
if( ! function_exists( 'istiqbal_excludecat' ) ) {
  function istiqbal_excludecat($query) {
  	if ( $query->is_home ) {
  ...
  add_filter('pre_get_posts', 'istiqbal_excludecat');
}

/* Excerpt Length */
class IstiqbalExcerpt {

  // Default length (by WordPress)
  public static $length = 55;

  // Output: istiqbal_excerpt('short');
  ...
  public static function length($new_length = 55) {
    IstiqbalExcerpt::$length = $new_length;
    add_filter('excerpt_length', 'IstiqbalExcerpt::new_length');
    IstiqbalExcerpt::output();
  }

  // Tells WP the new length
  public static function new_length() {
    if( isset(IstiqbalExcerpt::$types[IstiqbalExcerpt::$length]) )
      return IstiqbalExcerpt::$types[IstiqbalExcerpt::$length];
    else
      return IstiqbalExcerpt::$length;
  }
...
if( ! function_exists( 'istiqbal_excerpt' ) ) {
  function istiqbal_excerpt($length = 55) {
    IstiqbalExcerpt::length($length);
  }
}

if ( ! function_exists( 'istiqbal_new_excerpt_more' ) ) {
  function istiqbal_new_excerpt_more( $more ) {
    return ' ';
  }
  add_filter('excerpt_more', 'istiqbal_new_excerpt_more');
}

/* Tag Cloud Widget - Remove Inline Font Size */
if( ! function_exists( 'istiqbal_tag_cloud' ) ) {
  function istiqbal_tag_cloud($tag_string){
    return preg_replace("/style='font-size:.+pt;'/", '', $tag_string);
  }
  add_filter('wp_generate_tag_cloud', 'istiqbal_tag_cloud', 10, 3);
}

/* Password Form */
if( ! function_exists( 'istiqbal_password_form' ) ) {
  function istiqbal_password_form( $output ) {
    $output = str_replace( 'type="submit"', 'type="submit" class=""', $output );
    return $output;
  }
  add_filter('the_password_form' , 'istiqbal_password_form');
}


/* Widget Layouts */
if ( ! function_exists( 'istiqbal_footer_widgets' ) ) {
  function istiqbal_footer_widgets() {
...
/* WP Link Pages */
if ( ! function_exists( 'istiqbal_wp_link_pages' ) ) {
  function istiqbal_wp_link_pages() {
    $defaults = array(
      'before'           => '<div class="wp-link-pages">' . esc_html__( 'Pages:', 'istiqbal' ),
      'after'            => '</div>',
      'link_before'      => '<span>',
      'link_after'       => '</span>',
...
/* Metas */
if ( ! function_exists( 'istiqbal_post_metas' ) ) {
  function istiqbal_post_metas() {
  ...
  <div class="bp-top-meta">
    ...
    <div class="bp-author">
      <?php
      printf(
        '<span>'. esc_html__('by','istiqbal') .' <a href="%1$s" rel="author">%2$s</a></span>',
...
/* Author Info */
if ( ! function_exists( 'istiqbal_author_info' ) ) {
  function istiqbal_author_info() {
    ...
    $author_text = $author_text ? $author_text : esc_html__( 'Author', 'istiqbal' );
...
=============================================== */
if ( ! function_exists( 'istiqbal_comment_modification' ) ) {
  function istiqbal_comment_modification($comment, $args, $depth) {
  ...
  <<?php echo esc_attr($tag); ?> <?php comment_class('item ' . $comment_class .' ' ); ?> id="comment-<?php comment_ID() ?>">
   ...
   <article>
    <div id="div-comment-<?php comment_ID() ?>" class="istiqbal-comment">
    <?php endif; ?>
    ...
    <div class="comment-main-area">
      ...
      <em class="comment-awaiting-moderation"><?php echo esc_html__( 'Your comment is awaiting moderation.', 'istiqbal' ); ?></em>
      <?php endif; ?>
      <div class="comment-content">
        ...
        <div class="comments-reply">
          <?php
            ...
            'reply_text' => '<span class="comment-reply-link icofont icofont-reply-all">'.esc_html__( 'Reply', 'istiqbal' ).'</span>',
            'before' => '',
            'class'  => '',
            'depth' => $depth,
...
if( ! function_exists( 'istiqbal_move_comment_field' ) ) {
  add_filter( 'comment_form_fields', 'istiqbal_move_comment_field' );
  function istiqbal_move_comment_field( $fields ) {
...
/* Title Area */
if ( ! function_exists( 'istiqbal_title_area' ) ) {
  function istiqbal_title_area() {
    ...
    if ($istiqbal_meta && (!is_archive() || is_woocommerce_shop())) {
      $custom_title = $istiqbal_meta['page_custom_title'];
      if ($custom_title) {
    ...
    if ( is_home() && is_front_page() ) {
      echo esc_html__( 'Blog', 'istiqbal' );
    } elseif ( is_home() && !is_front_page() ) {
      single_post_title();
    } elseif ( is_search() ) {
      printf( esc_html__( 'Search Results for %s', 'istiqbal' ), '<span>' . get_search_query() . '</span>' );
    } elseif ( is_category() || is_tax() ){
      single_cat_title();
    } elseif ( is_tag() ){
      single_tag_title( esc_html__('Posts Tagged: ', 'istiqbal') );
    } elseif ( is_archive() ){
      if ( is_day() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'istiqbal' ), array( 'span' => array() ) ), get_the_date());
      } elseif ( is_month() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'istiqbal' ), array( 'span' => array() )), get_the_date( 'F, Y' ));
      } elseif ( is_year() ) {
        printf( wp_kses( __( 'Archive for <span>%s</span>', 'istiqbal' ), array( 'span' => array() ) ), get_the_date( 'Y' ));
      } elseif ( is_author() ) {
        printf( wp_kses( __( 'Posts by: <span>%s</span>', 'istiqbal' ), array( 'span' => array() ) ), get_the_author_meta( 'display_name', $wp_query->post->post_author ));
      } elseif( is_woocommerce_shop() ) {
        esc_html_e( 'Shop', 'istiqbal' );
      } elseif( is_product() ) {
        esc_html_e( 'Shop Single', 'istiqbal' );
      } elseif ( is_post_type_archive() ) {
        post_type_archive_title();
      } else {
        esc_html_e( 'Archives', 'istiqbal' );
      }
    } elseif( ( class_exists( 'WooCommerce' ) ) && ( is_product() ) ) {
       the_title();
    } elseif( is_singular( 'event' ) ) {
     esc_html_e('Event Single', 'istiqbal');
    } elseif( is_singular( 'service' ) ) {
      the_title();
    } elseif( is_singular( 'team' ) ) {
      esc_html_e('Team Single', 'istiqbal');
    }  elseif( is_404() ) {
      esc_html_e('404 Error', 'istiqbal');
    } elseif( $custom_title ) {
...
// Pingback Head
function istiqbal_pingback_header() {
  if ( is_singular() && pings_open() ) {
    printf( '<link rel="pingback" href="%s">' . "\n", get_bloginfo( 'pingback_url' ) );
  }
}
add_action( 'wp_head', 'istiqbal_pingback_header' );


/**
 * blog pagination function.
 */
if ( ! function_exists( 'istiqbal_posts_navigation' ) ) :
  function istiqbal_posts_navigation() {
 ...
 */
if ( ! function_exists( 'istiqbal_single_pagination' ) ) :
  function istiqbal_single_pagination() {
    $newer_post = cs_get_option('newer_post');
    $newer_post = ( $newer_post ) ? $newer_post : esc_html__('Next Post', 'istiqbal');
    $older_post = cs_get_option('older_post');
    $older_post = ( $older_post ) ? $older_post : esc_html__('Previous Post', 'istiqbal');
    the_post_navigation(
      array(
        'next_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Next Post', 'istiqbal' ) . '</span> ' .
          '<span class="screen-reader-text">' . esc_html( $newer_post ) . '</span> <br/>' .
          '<span class="post-title">%title</span>',
        'prev_text' => '<span class="meta-nav" aria-hidden="true">' . esc_html( 'Previous Post', 'istiqbal' ) . '</span> ' .
...
function istiqbal_unique_id( $prefix = '' ) {
  static $id_counter = 0;
...
if (!function_exists('istiqbal_header_function')) {
  function istiqbal_header_function(){
  ...
  $istiqbal_meta  = get_post_meta( $istiqbal_id, 'page_type_metabox', true );
  // Theme Layout Width
  $istiqbal_layout_width  = cs_get_option( 'theme_layout_width' );
  $theme_preloder  = cs_get_option( 'theme_preloder' );
  $istiqbal_layout_width_class = ( $istiqbal_layout_width === 'container' ) ? 'layout-boxed' : 'layout-full';
  // Header Style
  if ( $istiqbal_meta ) {
    $istiqbal_header_design  = $istiqbal_meta['select_header_design'];
    if ($istiqbal_header_design != 'theme') {
      $istiqbal_header_design = $istiqbal_header_design;
    } else {
  ...
  if (is_numeric($istiqbal_header_design)) {
  ...
  <body <?php body_class(); ?>>
     <?php wp_body_open(); ?>
  <div class="page-wrapper <?php echo esc_attr( $istiqbal_layout_width_class ); ?>"> <!-- #istiqbal-theme-wrapper -->
  <?php if( $theme_preloder ) {
   get_template_part( 'theme-layouts/header/preloder' );
    ...
    <header id="header" class="<?php echo esc_attr( $header_class ); ?>">
      <?php  if (is_numeric($istiqbal_header_design)) {
        $header_builder = new WP_Query(
          array(
            'post_type' => 'headerbuilder',
            'posts_per_page' => 1,
            'p' => $istiqbal_header_design,
            'orderby' => 'none',
            'order' => 'DESC'
          )
        );
    ...
    <?php
    // Title Area
    $istiqbal_need_title_bar = cs_get_option('need_title_bar');
    if ( !$istiqbal_need_title_bar ) {
