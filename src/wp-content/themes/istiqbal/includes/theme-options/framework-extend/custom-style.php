...
if ( ! function_exists( 'istiqbal_dynamic_styles' ) ) {
  function istiqbal_dynamic_styles() {

    // Typography
    $istiqbal_get_typography  = istiqbal_get_typography();
    ...
    // Logo
    $istiqbal_logo_top     = cs_get_option( 'istiqbal_logo_top' );
    $istiqbal_logo_bottom  = cs_get_option( 'istiqbal_logo_bottom' );
...
$layout_boxed = ( $bg_pattern === 'custom-pattern' ) ? 'background-image: url('. esc_url($custom_bg_pattern) .');' : 'background-image: url('. esc_url(ISTIQBAL_IMAGES) . '/patterns/'. $bg_pattern .'.png);';
?>
  .layout-boxed {
    <?php echo   wp_kses_data( $layout_boxed ); ?>
  }
...
if ($istiqbal_meta) {
  $transparent_header =  isset( $istiqbal_meta['transparency_header'] ) ? $istiqbal_meta['transparency_header'] : '' ;
  $transparent_menu_color = isset( $istiqbal_meta['transparent_menu_color'] ) ? $istiqbal_meta['transparent_menu_color'] : '' ;
  $transparent_menu_hover = isset( $istiqbal_meta['transparent_menu_hover_color'] ) ? $istiqbal_meta['transparent_menu_hover_color'] : '' ;
} else {
  $transparent_header = '';
...
<?php
  ...
  echo   wp_kses_data( $istiqbal_get_typography );
  $output = ob_get_clean();
  return $output;
 ...
 * Custom Font Family
 */
if ( ! function_exists( 'istiqbal_custom_font_load' ) ) {
  function istiqbal_custom_font_load() {
...
/* Custom Styles */
if( ! function_exists( 'istiqbal_custom_css' ) ) {
  function istiqbal_custom_css() {
    wp_enqueue_style('istiqbal-default-style', get_template_directory_uri() . '/style.css');
    $output  = istiqbal_custom_font_load();
    $output .= istiqbal_dynamic_styles();

    if( function_exists( 'istiqbal_compress_css_lines' ) ) {
      $custom_css = istiqbal_compress_css_lines( $output );
       wp_add_inline_style( 'istiqbal-default-style', $custom_css );
    }
    istiqbal_typography_fonts();
  }
  add_action( 'wp_enqueue_scripts', 'istiqbal_custom_css' );
}

/* Custom JS */
if( ! function_exists( 'istiqbal_custom_js' ) ) {
  function istiqbal_custom_js() {
    $output = cs_get_option( 'theme_custom_js' );
    if ( $output ) {
      wp_add_inline_script( 'jquery-migrate', $output );
    }
  }
  add_action( 'wp_enqueue_scripts', 'istiqbal_custom_js' );
}