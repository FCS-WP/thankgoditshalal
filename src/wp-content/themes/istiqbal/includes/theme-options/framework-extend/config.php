...
if (function_exists('cs_framework_init')) {
  ...
  require_once( ISTIQBAL_CS_FRAMEWORK . '/theme-options.php' );
  require_once( ISTIQBAL_CS_FRAMEWORK . '/theme-customizer.php' );
  require_once( ISTIQBAL_CS_FRAMEWORK . '/theme-metabox.php' );
   ...
   */
  if( ! function_exists( 'istiqbal_custom_font_upload' ) ) {
    function istiqbal_custom_font_upload( $db_value ) {
    ...
    add_action( 'cs_typography_family', 'istiqbal_custom_font_upload' );
  }
 ...
 * Check Custom Font
 */
if ( ! function_exists( 'istiqbal_custom_upload_font' ) ) {
  function istiqbal_custom_upload_font( $font ) {
 ...
 */
if ( ! function_exists( 'istiqbal_registered_sidebars' ) ) {
  function istiqbal_registered_sidebars() {
 ...
 */
if ( ! function_exists( 'istiqbal_typography_fonts' ) ) {
  function istiqbal_typography_fonts() {
    ...
    if ( ! empty( $embed_fonts ) ) {
      ...
      wp_enqueue_style( 'istiqbal-typography-fonts', esc_url( add_query_arg( 'family', urlencode( implode( '|', $query_args ) ) . $subsets, '//fonts.googleapis.com/css2' ) ), array(), null );
    }
...
/* Typography */
if ( ! function_exists( 'istiqbal_get_typography' ) ) {
  function istiqbal_get_typography() {
    ...
    if ( ! empty( $typography ) ) {
      foreach ( $typography as $font ) {
        if ( ! empty( $font['selector'] ) ) {
          ...
          $weight  = ( $font['font']['variant'] != 'regular' ) ? istiqbal_esc_string( $font['font']['variant'] ) : 400;
          $style   = istiqbal_esc_number( $font['font']['variant'] );
          ...
          $output .= ( ! empty( $font['size'] ) ) ? 'font-size: '. istiqbal_check_px( $font['size'] ) .';' : '';
