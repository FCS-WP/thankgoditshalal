...
$istiqbal_alt = get_post_meta($istiqbal_large_image, '_wp_attachment_image_alt', true);
$istiqbal_read_more_text = cs_get_option('read_more_text');
$istiqbal_read_text = $istiqbal_read_more_text ? $istiqbal_read_more_text : esc_html__( 'Read More', 'istiqbal' );
...
if ( 'gallery' == get_post_format() && ! empty( $istiqbal_post_type['gallery_post_format'] ) ) {
  $post_format_class = ' slider-post';
  $quote_output = '';
} elseif ( 'video' == get_post_format() && ! empty( $istiqbal_post_type['video_post_format'] ) ) {
  $post_format_class = ' video-post';
  $quote_output = '';
}  elseif ( 'quote' == get_post_format() && ! empty( $istiqbal_post_type['quote_post_format'] ) ) {
 ...
 <div <?php post_class('post'.$post_format_class); ?>>
  <?php
    ...
    <div class="entry-media post-slider"
        ...
        data-dots="true">
    <?php
      $istiqbal_ids = explode( ',', $istiqbal_post_type['gallery_post_format'] );
      foreach ( $istiqbal_ids as $id ) {
        $istiqbal_attachment = wp_get_attachment_image_src( $id, 'full' );
        $istiqbal_alt = get_post_meta($id, '_wp_attachment_image_alt', true);
        echo '<img src="'. $istiqbal_attachment[0] .'" alt="'. esc_attr( $istiqbal_alt ) .'" />';
      }
    ?>
   </div>
  <?php } elseif ( 'video' == get_post_format() && ! empty( $istiqbal_post_type['video_post_format'] ) ) { ?>
    <div class="entry-media video-holder">
        <img src="<?php echo esc_url( $istiqbal_large_image[0] ); ?>" alt="<?php echo esc_attr( $istiqbal_alt ); ?>">
        <a href="<?php echo esc_url( $istiqbal_post_type['video_post_format'] ); ?>?autoplay=1" class="video-btn" data-type="iframe">
            <i class="fi flaticon-video-player"></i>
        </a>
    </div>
   <?php }  elseif ( 'quote' == get_post_format() ) { ?>
      <div class="quote-icon"></div>
    <?php } elseif ( $istiqbal_large_image ) { ?>
    ...
    <ul class="entry-meta">
        <li>
          <?php if ( !in_array( 'date', $istiqbal_metas_hide ) ) { ?> 
        ...
        <li>
          <?php if ( !in_array( 'comment', $istiqbal_metas_hide ) ) { ?> 
    ...
    <div class="entry-details">
      <p><?php
            ...
            if(istiqbal_excerpt($blog_excerpt)) {
              istiqbal_excerpt($blog_excerpt);
            } else {
      ...
      <a href="<?php echo esc_url( get_permalink() ); ?>" class="theme-btn-s2">
        <?php echo esc_attr($istiqbal_read_text); ?>
      </a>
    </div>
</div>
